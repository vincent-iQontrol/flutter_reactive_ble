...
open class ReactiveBleClient(private val context: Context) : BleClient {
    ...
    override fun connectToDevice(deviceId: String, timeout: Duration) {
        val shouldBond = true // TODO: eventueel ophalen uit Flutter call
        allConnections.add(
            getConnection(deviceId, timeout, shouldBond)
                .subscribe({ result ->
                    when (result) {
                        is EstablishedConnection -> Unit
                        is EstablishConnectionFailure -> connectionUpdateBehaviorSubject.onNext(
                            ConnectionUpdateError(deviceId, result.errorMessage)
                        )
                    }
                }, { error ->
            ...
            when (connectionResult) {
                is EstablishedConnection -> {
                    if (rxBleClient.getBleDevice(connectionResult.deviceId).bluetoothDevice.bondState == BOND_BONDING) {
                        Single.error(Exception("Bonding is in progress, wait before executing operations."))
                    } else {
        ...
        getConnection(deviceId).flatMapSingle { connectionResult ->
            when (connectionResult) {
                ...
                is EstablishConnectionFailure ->
                    Single.just(CharOperationFailed(deviceId, "failed to connect ${connectionResult.errorMessage}"))
            }
    ...
    override fun setupNotification(deviceId: String, characteristic: UUID): Observable<ByteArray> {
        return getConnection(deviceId)
            .flatMap { connection -> setupNotificationOrIndication(connection, characteristic) }
            .flatMap { it }
    }
        ...
        getConnection(deviceId).flatMapSingle { connectionResult ->
            when (connectionResult) {
                is EstablishedConnection -> connectionResult.rxConnection.requestMtu(size)
                    .map { MtuNegotiateSuccesful(deviceId, it) }

                is EstablishConnectionFailure ->
                    Single.just(MtuNegotiateFailed(deviceId, "failed to connect ${connectionResult.errorMessage}"))
            }
    ...
    private fun getConnection(
        ...
        val connector = activeConnections.getOrPut(deviceId) {
            createDeviceConnector(device, timeout, shouldBond)
        }
    ...
    private fun executeWriteOperation(
        ...
        characteristic: UUID,
        value: ByteArray,
        bleOperation: RxBleConnection.(UUID, ByteArray) -> Single<ByteArray>
    ): Single<CharOperationResult> =
        getConnection(deviceId).flatMapSingle { connectionResult ->
            when (connectionResult) {
                is EstablishedConnection -> connectionResult.rxConnection
                    .bleOperation(characteristic, value)
                    .map { CharOperationSuccessful(deviceId, it.asList()) }
    ...
    private fun setupNotificationOrIndication(
        ...
        return when (deviceConnection) {
            ...
            is EstablishConnectionFailure -> Observable.just(Observable.empty())
        }
    }
    ...
    override fun requestConnectionPriority(
        ...
        getConnection(deviceId).switchMapSingle { connectionResult ->
            when (connectionResult) {
                is EstablishedConnection -> connectionResult.rxConnection
                    .requestConnectionPriority(priority.code, 2, TimeUnit.SECONDS)
                    .toSingle { RequestConnectionPrioritySuccess(deviceId) }

                is EstablishConnectionFailure ->
                    Single.just(RequestConnectionPriorityFailed(deviceId, connectionResult.errorMessage))
            }
